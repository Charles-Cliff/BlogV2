...
description: 'Claude Code实用指南与技巧汇总。涵盖安装配置、核心命令、MCP模块、子智能体、Hooks钩子等完整使用笔记'
image: '/images/2025-09/claude.webp'
...
Claude Code �?Anthropic 推出�?**AI 智能编程工具**，可在终端、IDE、桌面应用和浏览器中运行。它能直接读取代码库、执行命令、修改文件、管�?Git 工作流，并通过 MCP 协议连接外部服务�?
## 文档目录
...
# Windows 安装（Powershell�?irm https://claude.ai/install.ps1 | iex
...
<details>
<summary>键值与结构�?/summary>

| **`cleanupPeriodDays`** | 非活动时间超过此期间的会话在启动时被删除。设置为 **`0`** 立即删除所有会话。（默认�?0 天） |
| --- | --- |
| **`model`** | 覆盖 Claude Code 使用的默认模�?|
| **`alwaysThinkingEnabled`** | 为所有会话默认启�?[**扩展思�?*](https://code.claude.com/docs/zh-CN/common-workflows#use-extended-thinking-thinking-mode) ，默认开�?|
| **`autoUpdatesChannel`** | 配置 Claude Code 为自动更�?默认�?*`latest`** ，可改为**`stable`**  |
| **`statusLine` 结构�?* | 配置自定义状态行以显示上下文。请参阅 [**`statusLine` 文档**](https://code.claude.com/docs/zh-CN/statusline) |
| **`attribution` 结构�?* | 是否�?git 提交和拉取请求中包含 **`co-authored-by Claude`** 署名，默认开�?|
| **`defaultMode`** | 打开 Claude Code 时的默认[**权限模式](https://code.claude.com/docs/zh-CN/permissions#permission-modes)** 建议�?*plan** |
...
| **`ANTHROPIC_AUTH_TOKEN`** | **`Authorization`** 标头的自定义值（您在此处设置的值将�?**`Bearer`** 为前缀�?|
| --- | --- |
| **`ANTHROPIC_BASE_URL`** | 自定义请求端�?|
| **`CLAUDE_AUTOCOMPACT_PCT_OVERRIDE`** | 设置触发自动压缩的上下文容量百分比（1-100）。默认情况下，自动压缩在�?95% 容量时触发�?|
| **`CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION`** | 提示建议，设置为**`flase`** 禁用 |
| **`CLAUDE_CODE_MAX_OUTPUT_TOKENS`** | 为大多数请求设置最大输出令牌数。默认：32,000。最大：64,000。增加此值会减少�?[**自动压缩**](https://code.claude.com/docs/zh-CN/costs#reduce-token-usage) 触发之前可用的有效上下文窗口�?|
| **`CLAUDE_CODE_FILE_READ_MAX_OUTPUT_TOKENS`** | 覆盖文件读取的默认令牌限制。当您需要完整读取较大文件时很有用默认为**`16000`**，最�?*`100000`** |
| **`MAX_THINKING_TOKENS`** | 思考预算，默认�?1999（最大）0为禁用，Opus4.6由努力级别控�?|
| **`MCP_TIMEOUT`** | MCP 服务器启动的超时 建议**`60000`** |
| **`MCP_TOOL_TIMEOUT`** | MCP 工具执行的超�?建议**`120000`** |
| **`BASH_DEFAULT_TIMEOUT_MS`** | 长时间运行的 bash 命令的默认超�?建议**`300000`** |
| **`BASH_MAX_TIMEOUT_MS`** | 模型可以为长时间运行�?bash 命令设置的最大超�?建议**`600000`** |
| **`CLAUDE_CODE_EFFORT_LEVEL`** | Opus4.6加入的控制努力级�?值：**`low`**�?*`medium`**�?*`high`**（默认） |

</details>

<details>
<summary>个人配置示例�?026.1.31�?/summary>
...
| **命令** | **中文说明** | **示例** |
| --- | --- | --- |
| `claude` | 启动交互�?REPL 会话 | `claude` |
| `claude "query"` | 带初始提示启动会�?| `claude "explain this project"` |
| `claude -p "query"` | 单次查询后退出（适合脚本�?| `claude -p "review this code"` |
| **`*claude --dangerously-skip-permissions*`** | 彻底疯狂模式，给予所有权�?|  |
...
| **`claude plugin install context7@claude-plugins-official`** | 提供较新文档给AI，有效防止因训练数据落后�?幻觉" |
| **`claude -p "queryclaude plugin install csharp-lsp@claude-plugins-official`** | �?C# 项目提供丰富的代码智�?|
| **`npx claude-plugins install @anthropics/anthropic-agent-skills/document-skills`** | 文档处理套件合集，包括Excel、Word、PowerPoint和PDF功能 |
| **`npx claude-plugins install @anthropics/anthropic-agent-skills/example-skills`** | 展示多种能力的示例技能集合，包括技能创建、MCP构建、视觉设计、算法艺术、内部沟通、网页测试、工件构建、Slack GIF和主题样�?|
...
| **`npx skills add [https://github.com/vercel-labs/agent-skills](https://github.com/vercel-labs/agent-skills) --skill vercel-react-best-practices`** | 由Vercel维护的React和Next.js应用综合性能优化指南。包�?个类别的57条规则，按影响优先级排序，指导自动重构和代码生成�?|
| **`npx skills add [https://github.com/nextlevelbuilder/ui-ux-pro-max-skill](https://github.com/nextlevelbuilder/ui-ux-pro-max-skill) --skill ui-ux-pro-max`** | 设计很棒�?原仓库：https://github.com/nextlevelbuilder/ui-ux-pro-max-skill |
...
| 在UnityPackageManager �?*`https://github.com/Besty0728/Unity-Skills.git?path=/SkillsForUnity`** | Unity-Skills，本人开�?https://github.com/Besty0728/Unity-Skills |
...
| `/context` | 查看当前上下文窗口使用情�?|
| `/cost` | 显示当前会话�?Token 用量与费�?|
| `/stats` | 查看使用统计信息 |
...
| `/fork` | 分支对话，探索不同方向而不丢失上下�?|
| `/clear` | 清空对话历史，重新开�?|
| **`/rewind`** | **回退到上一个检查点（撤销更改�?* |
| `/export` | 导出对话记录 |
| `/vim` | 启用 Vim 编辑模式 |
| **`/ide`** | **连接�?IDE（VS Code / JetBrains�?* |
| `/sandbox` | 启用沙盒隔离模式 |
...
| **模型别名** | **行为** |
| --- | --- |
| **`default`** | 推荐的模型设置，取决于你的账户类�?|
| **`sonnet`** | 使用最新的 Sonnet 模型（当前为 Sonnet 4.5）用于日常编码任�?|
| **`opus`** | 使用最新的 Opus 模型（当前为 Opus 4.6）用于复杂推理任�?|
| **`haiku`** | 使用快速高效的 Haiku 模型用于简单任�?|
| **`sonnet[1m]`** | 使用 Sonnet 和[**100 �?token 上下文窗�?*](https://platform.claude.com/docs/en/build-with-claude/context-windows#1m-token-context-window)用于长会�?|
| **`opusplan` 推荐** | 特殊模式，在计划模式中使�?**`opus`**，然后在执行时切换到 **`sonnet`** |

| **模型** | **适用场景** | **输入 / 1M tokens** | **输出 / 1M tokens** |
| --- | --- | --- | --- |
| **Haiku** | 简单任务、快速探索、子智能�?| $1.00 | $5.00 |
| **Sonnet**（默认） | 日常编程、功能开发、Bug 修复 | $3.00 | $15.00 |
| **Opus** | 复杂推理、架构设计、安全分�?| $5.00 | $25.00 |

<aside>
💡

**选择建议�?* 日常�?Sonnet，子智能体探索用 Haiku（省 10-20 倍费用），只有架构设计或复杂调试才用 Opus�?
</aside>
...
### 配置文件层级（优先级从高到低�?
1. **企业�?*：`/etc/claude-code/managed-settings.json` �?全公司强制策略，用户不可覆盖
2. **CLI 标志**：命令行参数 �?当前会话
3. **本地项目**：`.claude/settings.local.json` �?个人，仅当前项目（应加入 .gitignore�?4. **共享项目**：`.claude/settings.json` �?团队共享，提交到 Git
5. **用户�?*：`~/.claude/settings.json` �?所有项�?
***个人开发直接使�?�?即可***

### [CLAUDE.md](http://CLAUDE.md) 记忆文件

用于持久化项目上下文，让 Claude 每次会话都能了解你的项目�?
- `~/.claude/[CLAUDE.md](http://CLAUDE.md)` �?全局，适用所有项�?- `./[CLAUDE.md](http://CLAUDE.md)` �?项目级，团队共享
- `./[CLAUDE.local.md](http://CLAUDE.local.md)` �?项目本地，个人笔�?- `.claude/rules/` �?分类规则目录（[testing.md](http://testing.md)、[security.md](http://security.md) 等）
<details>
...
- 优先简洁实用，避免过度设计
- 保持现有代码风格一致�?# 技术栈偏好
- Unity: C#，遵循Unity最佳实�?- Web: TypeScript优先，Next.js/Astro
- Python: 类型注解，async优先
- 包管�? pnpm (Node) / uv (Python)
# 工作习惯
- 修改代码前先阅读理解现有逻辑
- 提交信息使用中文，格式：类型: 简�?- 遇到不确定的需求主动询�?```
...
标准工作流：
1. 分析需�?�?2. 获取上下�?�?3. 选择工具 �?4. 执行任务 �?5. 验证质量 �?6. 存储知识

研究-计划-实施模式�?研究阶段: 读取文件理解问题，禁止编�?计划阶段: 创建详细计划
实施阶段: 实施解决方案
验证阶段: 运行测试验证
提交阶段: 创建提交和文�?
## MANDATORY TOOL STRATEGY
════════════════════════�?
任务开始前必须执行�?1. memory 查询相关概念
...
══════════════════�?
编码前强制要求：
- 无明确编写命令禁止编�?- 无明确授权禁止修改文�?- 必须先完成sequential-thinking分析

## QUALITY STANDARDS
══════════════════�?
工程原则：SOLID、DRY、关注点分离
代码质量：清晰命名、合理抽象、必要注�?性能意识：算法复杂度、内存使用、IO优化
测试思维：可测试设计、边界条件、错误处�?
## SUBAGENT SELECTION
════════════════════

必须主动调用合适子代理�?- C#/.NET项目 �?csharp-pro  
- 前端开�?�?frontend-developer
...
| **模式** | **中文说明** | **适用场景** |
| --- | --- | --- |
| `default` | 首次使用每个工具时提示确�?| 日常开�?|
| `acceptEdits` | 自动批准文件编辑，Bash 仍需确认 | 信任项目 |
| `plan` | 仅分析，禁止执行或编�?| 代码审查 |
| `bypassPermissions` | 跳过所有确认提示（谨慎使用�?| CI/CD 自动�?|
...
Hooks 在特定时�?**确定�?* 执行 Shell 命令，不依赖模型判断，保证每次都会运行�?
### 可用事件
...
## 🔌 MCP（模型上下文协议�?
MCP �?Claude Code 连接外部工具、数据库、API 和服务�?
### 常用命令
...
description: Unity 代码静态审查专家，审查 Unity C# 代码的性能、GC、正确性、线程安全、序列化陷阱和可维护性。只读审查，不修改文件�?model: sonnet
---
...
## 🎨 Skills 技能系�?
与斜杠命令不同，Skills �?**模型自动调用** �?�?Claude 根据上下文自动识别并应用�?
### 创建 Skill

�?`.claude/skills/my-skill/[SKILL.md](http://SKILL.md)` 中定义：

```markdown
---
name: code-reviewer
description: 代码审查专家，检查安全漏洞、性能问题和最佳实�?allowed-tools: Read, Grep, Glob
---
# 代码审查专业知识
审查时检查以下方面：
- 输入验证与注入攻�?- 认证与授权缺�?- 性能与内存问�?```
...
1. 子智能体探索�?**Haiku**（比 Opus 便宜 10-20 倍）
2. 开�?**Prompt Caching**（默认开启）
3. 设置 `--max-turns` 防止失控对话
4. 定期执行 `/compact` 压缩上下�?5. 设置 `--max-budget-usd` 限制单次花费上限
...
╔══════════════════════════════════════════════════╗
�?         CLAUDE CODE 快速参�?                    �?╠══════════════════════════════════════════════════╣
�?模型选择                                         �?�? haiku    �?简单任务、探�?           ($1/$5/M)   �?�? sonnet   �?日常编程（默认）          ($3/$15/M)  �?�? opus     �?架构设计、复杂推�?       ($5/$25/M)  �?╠══════════════════════════════════════════════════╣
�?必备命令                                         �?�? /compact   压缩上下文（50%时执行）               �?�? /cost      查看费用                              �?�? /model     切换模型                              �?�? /status    查看状�?                             �?�? /init      初始化项目配�?                       �?�? /mcp       管理外部集成                          �?╠══════════════════════════════════════════════════╣
�?前缀速查                                         �?�? # 消息     �?保存到记�?                         �?�? @ 路径     �?引用文件                            �?�? ! 命令     �?执行 Bash                           �?�? & 任务     �?云端异步执行                        �?╠══════════════════════════════════════════════════╣
�?决策规则                                         �?�? 简单任务？       �?Haiku                         �?�? 复杂推理�?      �?Opus                          �?�? 其他所有？       �?Sonnet                        �?�? 必须每次执行�?  �?Hook（非提示�?               �?�? 自动应用知识�?  �?Skill（非命令�?              �?�? 需要隔离上下文�?�?子智能体                      �?╚══════════════════════════════════════════════════╝
```
